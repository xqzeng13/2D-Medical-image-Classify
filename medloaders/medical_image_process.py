...
def load_crop_to_mage(data_label, type=None, resample=None,
                       viz3d=False, to_canonical=False, rescale=None, normalization='full_volume_mean',
                       clip_intenisty=True, crop_size=(0, 0, 0), crop=(0, 0, 0), ):
    # img_nii = nib.load(path)

    if to_canonical:
        img_nii = nib.as_closest_canonical(data_label)

    if resample is not None:
        img_nii = resample_to_output(data_label, voxel_sizes=resample)
